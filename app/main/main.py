...
class PsyApplication(QMainWindow):
    ...
    def compile(self):
        ...
        with open(compile_file_name, "w") as f:
            ...
            print(f"% Department of Psychology, SooChow University",file=f)
            print(f"% zhangyang873@gmail.com Or yzhangpsy@suda.edu.cn",file=f)
            print(f"% {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",file=f)
            print(f"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%",file=f)
            print(f"%      begin      ",file=f)
            print(f"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%",file=f)
            print(f"try",file=f)
            print(f"catch {cFilenameOnly}_error",file=f)
            print(f"\tsca;",file=f)
