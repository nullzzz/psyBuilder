...
from PyQt5.QtWidgets import QTableWidget, QAbstractItemView, QShortcut, QAction, QMenu, QApplication

from app.center.widget_tabs.timeline.widget_icon import WidgetIcon
from app.func import Func
from app.info import Info
from lib import TableWidget, VarLineEdit, MessageBox
...
class TimelineTable(TableWidget):
    ...
    def addTimeline(self, item: QTableWidgetItem):
        ...
        if self.edit_row == item.row() and item.column() == 1 and item.text():
            ...
            if validity == Info.TimelineNameRight:
                ...
                item.setText(self.old_timeline_name)
                MessageBox.information(self, 'Warning', "Can't refer different type widget.")
            elif validity == Info.TimelineParentError:
                item.setText(self.old_timeline_name)
                MessageBox.information(self, 'Warning', "Can't refer this timeline.")
            else:
    ...
    def paste_data(self):
        ...
        # 解析出0列
        if not col_length:
            MessageBox.information(self, 'Warning', "Cols split by '\\t' is zero!")
            return
    ...
    def clear_data(self):
        ...
        for select_range in select_ranges:
            ...
            if 0 in range(left_column, right_column + 1):
                MessageBox.information(self, "warning", "Weight can't be cleared.")
                return
